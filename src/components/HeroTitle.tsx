...
const HeroTitle = () => {
  ...
  return (
    <div className="relative w-full md:w-fit h-full flex gap-3 flex-col md:flex-row">
      <h1 className="text-4xl text-white sm:text-4xl  uppercase">
        The best in
      </h1>
      <div className="relative text-center md:text-left overflow-hidden h-[50px]">
        {titles.map((title, index) => (
          <div
            ...
            key={index}
            className="transition-transform uppercase justify-center md:justify-start duration-1000 text-white md:text-left text-center font-bold text-4xl h-[48px]  flex ">
            {title}
          </div>
        ))}
      </div>
